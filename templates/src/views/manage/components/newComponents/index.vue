<template>
    <div>
        <a-modal :title="titles" :visible="visible" @cancel="init" :maskClosable="false" width="700px" footer="">
            <a-form :form="form" @submit="handleSubmit">
                ...
                <a-form-item label="组件名称" v-bind="waiformItemLayout">
                    <a-input
                        ...
                        v-decorator="['name', { rules: [{ required: true, message: '请输入组件名称，例如DNS' }] }]"
                    />
                </a-form-item>
                <a-form-item label="组件介绍" v-bind="waiformItemLayout">
                    <a-textarea
                        ...
                        v-decorator="['desc', { rules: [{ required: true, message: '请输入组件介绍' }] }]"
                    />
                </a-form-item>
                <a-form-item label="生成链接格式" v-bind="waiformItemLayout">
                    <a-input
                        ...
                        v-decorator="['payload', { rules: [{ required: true, message: '请输入生成链接格式' }] }]"
                    />
                </a-form-item>
                ...
                <a-form-item label="公开组件" v-bind="waiformItemLayout">
                    <a-radio-group
                        v-decorator="['is_private', { rules: [{ required: true, message: '请选择是否公开组件' }] }]"
                    >
                ...
                <a-form-item v-bind="layoutBottomWithOutLabel">
        ...
        configInfo(values) {
            ...
            Object.keys(values).forEach((key) => {
                if (key.includes('item_name')) {
                    let arr = []
                    Object.keys(values.config).forEach((childkey) => {
            ...
            values.file_name = Array.isArray(values.file_name)
                ? values.file_name[0].response.data.file_name
                : this.content.file_name
        },
        callingInter(values) {
            if (this.content) {
                Service.update_template({ ...values, template_id: this.content.id }).then((item) => {
                    this.init()
                    this.$parent.initData()
                })
            } else {
                Service.templatesManage(values).then((item) => {
                    this.init()
                    this.$parent.initData()
                })
            }
